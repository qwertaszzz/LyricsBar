...
static BOOL UsesChinese(void) {
    NSString *language=NSLocale.preferredLanguages.firstObject ?: @"en";
    return [language hasPrefix:@"zh"];
}
static NSString *L(NSString *zh, NSString *en) { return UsesChinese() ? zh : en; }
...
- (void)applicationDidFinishLaunching:(NSNotification *)n {
    ...
    NSMenuItem *hint = [[NSMenuItem alloc] initWithTitle:L(@"歌词宽度会随内容和菜单栏空间自动调整", @"Lyric width adapts to the available menu-bar space") action:nil keyEquivalent:@""];
    hint.enabled = NO; [menu addItem:hint]; [menu addItem:NSMenuItem.separatorItem];
    NSMenuItem *settings = [[NSMenuItem alloc] initWithTitle:L(@"字体设置…", @"Font Settings…") action:@selector(openSettings:) keyEquivalent:@","];
    settings.target = self; [menu addItem:settings]; [menu addItem:NSMenuItem.separatorItem];
    NSMenuItem *quit = [[NSMenuItem alloc] initWithTitle:L(@"退出 LyricsBar", @"Quit LyricsBar") action:@selector(quit:) keyEquivalent:@"q"];
...
- (void)tick:(NSTimer *)timer {
    ...
    if (!t) { _trackID=@""; _lines=@[]; [self show:L(@"等待 Apple Music…", @"Waiting for Apple Music…")]; return; }
...
- (void)openSettings:(id)sender {
    if (!_settingsPanel) {
        ...
        _settingsPanel.title=L(@"LyricsBar 字体设置", @"LyricsBar Font Settings"); _settingsPanel.releasedWhenClosed=NO;
        ...
        [_fontPopup addItemsWithTitles:UsesChinese() ? @[@"系统字体",@"等宽字体",@"圆体",@"宋体"] : @[@"System",@"Monospaced",@"Rounded",@"Songti"]];
        ...
        _boldCheck=[NSButton checkboxWithTitle:L(@"半粗体", @"Semibold") target:self action:@selector(fontSettingChanged:)];
        ...
        NSGridView *grid=[NSGridView gridViewWithViews:@[
            @[label(L(@"字体：", @"Font:")),_fontPopup], @[label(L(@"字号：", @"Size:")),_sizeSlider], @[label(L(@"粗细：", @"Weight:")),_boldCheck], @[label(L(@"颜色：", @"Color:")),_colorWell]
        ]];
        grid.translatesAutoresizingMaskIntoConstraints=NO; grid.rowSpacing=14; grid.columnSpacing=12; [view addSubview:grid];
        NSTextField *note=[NSTextField wrappingLabelWithString:L(@"修改会立即应用并自动保存。长歌词仍会根据菜单栏可用宽度省略。", @"Changes apply immediately and are saved automatically. Long lyrics are truncated to fit the available menu-bar space.")];
